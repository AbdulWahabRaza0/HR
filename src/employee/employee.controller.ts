...
export class EmployeeController {
  ...
  @ApiOperation({
    summary: 'Get my updated data',
    description: 'Retrieve my data from the system.',
  })
  @ApiResponse({
    status: 200,
    description: 'Successfully retrieved my data.',
  })
  @ApiResponse({ status: 500, description: 'Internal Server Error.' })
  async me(@Req() req: Request, @Res() res: Response) {
    try {
      const meEmployee = await this.employeeService.findUserByReq(req);
      return res.status(200).json({ meEmployee });
    } catch (e) {
      console.log(e);
      res.status(500).json(e);
    }
