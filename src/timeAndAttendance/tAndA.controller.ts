...
export class TANDAController {
  ...
  async absent(@Req() req: any, @Res() res: Response) {
    try {
      ...
      myAttendanceRegister.presentHours.push({
        hours: 0,
        minutes: 0,
        timestamp: Date.now(),
      });
  ...
  @ApiResponse({ status: 500, description: 'Internal Server Error' })
  async getSpecificLeaveRequests(@Req() req: any, @Res() res: Response) {
    try {
      const myUser = await this.employeeService.findUserByReq(req);
      if (!myUser.TAID) {
        res.status(401);
        throw new Error('insuffiecient details');
      }

      const myTimeAndAttend = await this.TimeAndAttendance.findById(
        myUser?.TAID,
      );
      if (!myTimeAndAttend) {
        res.status(401);
      ...
      const data = [];
      for (const id of myTimeAndAttend?.LRID) {
        const leave = await this.LeaveReq.findById(id);
        if (!leave) {
          res.status(401);
  ...
  async deleteLeaveRequest(
    ...
    try {
      ...
      return res.status(201).json(newLeaveReq);
    } catch (e) {
      console.log(e);
      res.status(500).json('Invalid Error');
    }
